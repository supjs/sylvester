...
export class Polygon {
  ...
  translate(vector) {
    let P = vector.elements || vector;
    this.vertices.each(function (node) {
      let E = node.data.elements;
  ...
  scale(k, point) {
    ...
    let anchor = this.vertices.first.data;
  ...
  trianglesForSurfaceIntegral() {
    ...
    let triangles = [];
    let firstVertex = this.vertices.first.data;
    let plane = this.plane;
    this.vertices.each(function (node, i) {
      if (i < 2) {
        return;
      }
      let points = [firstVertex, node.prev.data, node.data];
  ...
  area() {
    if (this.isTriangle()) {
      ...
      let trigs = this.trianglesForSurfaceIntegral(),
        area = 0;
      let i = trigs.length;
      while (i--) {
  ...
  removeVertex(vertex) {
    ...
    let prev = node.prev,
      next = node.next;
    let prevWasConvex = prev.data.isConvex(this);
    let nextWasConvex = next.data.isConvex(this);
    ...
    if (prevWasConvex !== prev.data.isConvex(this)) {
      if (prevWasConvex) {
        ...
        this.reflexVertices.append(new LinkedListNode(prev.data));
      } else {
        this.reflexVertices.remove(this.reflexVertices.withData(prev.data));
        this.convexVertices.append(new LinkedListNode(prev.data));
      }
    }
    ...
    if (nextWasConvex !== next.data.isConvex(this)) {
      if (nextWasConvex) {
        ...
        this.reflexVertices.append(new LinkedListNode(next.data));
      } else {
        this.reflexVertices.remove(this.reflexVertices.withData(next.data));
        this.convexVertices.append(new LinkedListNode(next.data));
      }
    }
    return this;
  }
  ...
  hasEdgeContaining(point) {
    ...
    let success = false;
    this.vertices.each(function (node) {
      if (new LineSegment(node.data, node.next.data).contains(P)) {
        success = true;
      }
    });
    return success;
  ...
  setVertices(points, plane) {
    ...
    let i = pointSet.length,
      newVertex;
    while (i--) {
      newVertex = pointSet[i].isConvex ? pointSet[i] : new PolygonVertex(pointSet[i]);
      this.vertices.prepend(new LinkedListNode(newVertex));
    }
    this.clearCache();
  ...
  populateVertexTypeLists() {
    ...
    this.vertices.each(function (node) {
      ...
      // LinkedListNode class wraps each vertex so it can belong to
      // many linked lists.
      self[node.data.type(self) + 'Vertices'].append(new LinkedListNode(node.data));
    });
  }
  ...
  copyVertices() {
    this.clearCache();
    this.vertices.each(function (node) {
      node.data = new PolygonVertex(node.data);
    });
    this.populateVertexTypeLists();
  }
